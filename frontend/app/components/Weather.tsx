...
const Item: React.FC<ItemProps> = ({ leadingIcon, title, value, unit }) => (
  <div className="flex flex-row items-center justify-between p-2">
    ...
    <p className="text-base"><span className="font-bold">{value}</span> <span>{unit}</span></p>
  </div>
);

export const Weather = () => (
  <div className="rounded-md flex flex-col border-2 border-gray-200 p-4">
    <Item title={"33°C"} value={"Cloudy"} unit={""}/>
    <Item title={"Humidity"} value={"50"} unit={"%"} leadingIcon={<WiHumidity className="text-xl"/>}/>
